...
export default async function EditImagePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!session) notFound();
  const [image, limits] = await Promise.all([
    managedImageDetail(env, session.user_id, id),
    ...
    <main className="mx-auto max-w-4xl space-y-4">
      ...
      <Card className="overflow-hidden">
        <CardHeader className="gap-4 sm:grid-cols-[1fr_auto]">
          <div>
            <p className="text-sm font-bold tracking-[0.16em] text-primary">{t("image.eyebrow")}</p>
            <CardTitle className="mt-1 text-3xl">{image.title ?? t("common.untitled")}</CardTitle>
            <CardDescription className="mt-2">
              <a className="text-primary hover:underline" href={`/${image.code}`}>
                {t("image.openShared")}
              </a>
            </CardDescription>
          </div>
          <form
            ...
            method="post"
            action={`/manage/images/${id}/delete`}
          >
        ...
        <CardContent className="space-y-8">
          ...
          <div className="flex flex-wrap gap-x-5 gap-y-1 text-sm text-muted-foreground">
            ...
            <span>{size}</span>
            <span>
              <LocalDateTime value={expires} /> {t("common.until")}
            </span>
          </div>
