...
export async function createCheckout(request: Request, env: CloudflareEnv): Promise<Response> {
  ...
  if (existingSubscription.plan === "plus")
...
export function plusCheckoutSessionParams({
  customerId,
  userId,
  priceId,
  baseUrl,
...
export async function createPortal(request: Request, env: CloudflareEnv): Promise<Response> {
  ...
  if (!user?.stripe_customer_id)
    return new Response(null, { status: 303, headers: { location: "/plus" } });
...
export async function handleStripeWebhook(request: Request, env: CloudflareEnv): Promise<Response> {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (typeof checkout.customer === "string" && checkout.client_reference_id) {
      await env.DB.prepare("UPDATE users SET stripe_customer_id = ? WHERE id = ?")
        .bind(checkout.customer, checkout.client_reference_id)
        .run();
    }
    ...
    if (subscriptionId) {
      const graceUntil =
        event.type === "invoice.payment_failed" ? Date.now() + SEVEN_DAYS_MS : null;
      await env.DB.prepare(
        `UPDATE subscriptions SET grace_until = ?, last_payment_event_created = ?, updated_at = ?
          WHERE stripe_subscription_id = ? AND last_payment_event_created <= ?`,
      )
        .bind(graceUntil, event.created * 1000, Date.now(), subscriptionId, event.created * 1000)
        .run();
    }
  }
  ...
  await enqueuePendingRetentionJobs(env);
  await env.DB.prepare(
    "INSERT INTO stripe_events (event_id, event_type, event_created, processed_at) VALUES (?, ?, ?, ?)",
  )
    .bind(event.id, event.type, event.created * 1000, Date.now())
    .run();
  return Response.json({ received: true }, { headers: { "cache-control": "no-store" } });
}
...
async function syncSubscription(
  ...
  const synced = await env.DB.prepare(
    ...
    .bind(
      ...
      subscription.cancel_at ? subscription.cancel_at * 1000 : null,
  ...
  if (
    (synced.meta.changes ?? 0) === 1 &&
    (subscription.status === "active" || subscription.status === "trialing")
  ) {
    await env.DB.prepare(
      `INSERT OR IGNORE INTO retention_jobs
...
async function enqueuePendingRetentionJobs(env: CloudflareEnv): Promise<void> {
  const jobs = await env.DB.prepare(
    "SELECT image_id FROM retention_jobs WHERE status IN ('pending', 'failed') AND attempts < 5 ORDER BY updated_at LIMIT 50",
  ).all<{ image_id: string }>();
