...
export async function managedAlbums(env: CloudflareEnv, userId: string): Promise<AlbumRow[]> {
  const rows = await env.DB.prepare(
    ...
    .bind(Date.now(), Date.now(), userId, PLUS_PLAN.albums)
    .all<AlbumRow>();
  return rows.results;
}

export async function managedAlbumDetail(
  env: CloudflareEnv,
  userId: string,
  id: string,
...
export async function findActiveAlbumByCode(
  env: CloudflareEnv,
  code: string,
): Promise<AlbumDetail | null> {
...
export async function createAlbum(request: Request, env: CloudflareEnv): Promise<Response> {
  ...
  const count = await env.DB.prepare(
    ...
    .first<{ count: number }>();
  if ((count?.count ?? 0) >= limits.albums)
    return formError("/manage/albums/new", "album_limit_reached");
  ...
  const statements = [
    ...
    env.DB.prepare(
      "INSERT INTO short_links (code, target_type, target_id, created_at) VALUES (?, 'album', ?, ?)",
    ).bind(code, id, now),
...
export async function updateAlbum(
  ...
  if (
    ...
    (visibility !== album.visibility || changesPassphrase)
  )
  ...
  const membershipChanged = !sameSet(
    existing.results.map((row) => row.image_id),
    requestedIds,
  );
  ...
  const retained = existing.results
    .map((row) => row.image_id)
    .filter((imageId) => desired.has(imageId));
  ...
  let passphraseRecord = {
    ...
    passphrase_iterations: null as number | null,
  };
    ...
    if (changesPassphrase) {
      ...
      passphraseRecord =
        (await env.DB.prepare(
          ...
          .bind(id)
          .first<typeof passphraseRecord>()) ?? passphraseRecord;
    }
  }
  await env.DB.batch([
    env.DB.prepare(
      ...
      passphraseRecord.passphrase_hash,
      passphraseRecord.passphrase_iterations,
      album.access_version + (accessChanged ? 1 : 0),
      id,
      session.user_id,
    ),
    env.DB.prepare("DELETE FROM album_images WHERE album_id = ?").bind(id),
    ...orderedIds.map((imageId, position) =>
      env.DB.prepare(
        "INSERT INTO album_images (album_id, image_id, position, added_at) VALUES (?, ?, ?, ?)",
      ).bind(id, imageId, position, now),
    ),
...
export async function reorderAlbum(
  ...
  if (!(await ownedAlbum(env, session.user_id, id)))
...
export async function deleteAlbum(
  ...
  await env.DB.batch([
    env.DB.prepare(
      "UPDATE albums SET deleted_at = ?, updated_at = ? WHERE id = ? AND owner_user_id = ? AND deleted_at IS NULL",
    ).bind(now, now, id, session.user_id),
    env.DB.prepare(
      "UPDATE short_links SET retired_at = ? WHERE code = ? AND retired_at IS NULL",
    ).bind(now, album.code),
    env.DB.prepare("DELETE FROM album_images WHERE album_id = ?").bind(id),
  ]);
