import { decodeBase64Url, encodeBase64Url, hmacSha256, pbkdf2Sha256, randomToken, sha256, timingSafeEqual } from "./crypto";
...
export function validPassphrase(value: FormDataEntryValue | null): value is string {
  return typeof value === "string" && Array.from(value).length >= 4 && Array.from(value).length <= 100;
}
...
export async function authorizeTarget(
  ...
  const session = sessionToken
    ? await env.DB.prepare("SELECT id, user_id FROM sessions WHERE token_hash = ? AND expires_at > ?")
        .bind(await sha256(sessionToken), Date.now()).first<{ id: string; user_id: string }>()
    : null;
  ...
  const grant = await env.DB.prepare(`SELECT id FROM share_access_grants
...
export async function authorizeAlbumForImage(
  ...
  const album = await env.DB.prepare(`SELECT a.id, a.owner_user_id, a.visibility, a.access_version, s.code
    ...
    .bind(albumCode, imageId).first<AccessControlledRow>();
...
export async function unlockTarget(
  ...
  if (!await consumeAttempt(request, env, target.code)) return redirectError(redirectPath, "too_many_attempts");

  const table = targetType === "image" ? "images" : "albums";
  const record = await env.DB.prepare(`SELECT passphrase_salt, passphrase_hash, passphrase_iterations FROM ${table} WHERE id = ?`)
    .bind(target.id).first<PassphraseRecord>();
  if (!record || !await verifyPassphrase(passphrase, record)) return redirectError(redirectPath, "invalid_passphrase");

  const now = Date.now();
  const token = randomToken();
  await env.DB.prepare(`DELETE FROM share_access_grants WHERE id IN (
      SELECT id FROM share_access_grants WHERE expires_at <= ? LIMIT 500
    )`).bind(now).run();
  await env.DB.prepare(`INSERT INTO share_access_grants
      (id, token_hash, target_type, target_id, access_version, created_at, expires_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)`)
    .bind(crypto.randomUUID(), await sha256(token), targetType, target.id, target.access_version, now, now + GRANT_LIFETIME_MS)
    .run();
...
export async function revokeTargetGrants(env: CloudflareEnv, targetType: TargetType, targetId: string): Promise<void> {
...
async function verifyPassphrase(passphrase: string, record: PassphraseRecord): Promise<boolean> {
  if (!record.passphrase_salt || !record.passphrase_hash || !record.passphrase_iterations) return false;
  const salt = decodeBase64Url(record.passphrase_salt);
  const expected = decodeBase64Url(record.passphrase_hash);
  if (!salt || !expected || record.passphrase_iterations < 100_000 || record.passphrase_iterations > PASSPHRASE_ITERATIONS) return false;
  return timingSafeEqual(await pbkdf2Sha256(passphrase, salt, record.passphrase_iterations), expected);
}

async function consumeAttempt(request: Request, env: CloudflareEnv, code: string): Promise<boolean> {
  ...
  await env.DB.prepare(`DELETE FROM share_access_attempts WHERE id IN (
      SELECT id FROM share_access_attempts WHERE created_at <= ? LIMIT 500
    )`).bind(windowStart).run();
  const result = await env.DB.prepare(`INSERT INTO share_access_attempts (ip_hash, code, created_at)
      SELECT ?, ?, ? WHERE (
        SELECT COUNT(*) FROM share_access_attempts WHERE ip_hash = ? AND code = ? AND created_at > ?
      ) < ?`)
    .bind(ipHash, code, now, ipHash, code, windowStart, ATTEMPT_LIMIT).run();
...
function redirectError(path: string, error: string): Response {
  return new Response(null, { status: 303, headers: { location: `${path}?error=${error}`, "cache-control": "no-store" } });
}
